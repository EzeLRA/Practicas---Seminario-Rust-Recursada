...
impl Duenio {
    ...
    pub fn es_igual_a(&self, d: &Duenio) -> bool {
        return (&self.nombre == d.get_nombre())
            && (&self.direccion == d.get_direccion())
            && (self.telefono == d.get_num_telefono());
    }
...
impl Mascota {
    ...
    pub fn es_igual_a(&self, m: &Mascota) -> bool {
        return (&self.nombre == m.get_nombre())
            && (self.edad == m.get_edad())
            && (self.tipo.es_igual_a(&m.get_tipo()))
            && (self.duenio.es_igual_a(&m.get_duenio()));
    }
...
impl Atencion {
    ...
    pub fn es_igual_a(&self, ate: &Atencion) -> bool {
        ...
        return self.mascota.es_igual_a(&ate.get_mascota())
            && (self.diagnostico == ate.diagnostico)
            && (self.tratamiento == ate.tratamiento)
            && (tiene_fecha);
    }
    ...
    pub fn new(
        ...
        diagnostico_in: &str,
        tratamiento_in: &str,
        proxima_visita_in: Option<Fecha>,
    ) -> Atencion {
        Atencion {
            mascota: mascota_in,
...
impl Veterinaria {
    ...
    pub fn es_igual_a(&self, v: &Veterinaria) -> bool {
        return (&self.nombre == v.get_nombre())
            && (&self.direccion == v.get_direccion())
            && (self.id == v.get_id());
    }
    /*
...
mod testing_ejercicio9 {
    ...
    fn operatoria_mascotas() {
        ...
        //Atendiende un gato
        assert!(
            v.atender_mascota().is_some_and(|ani| {
    ...
    fn operar_atenciones() {
        ...
        assert!(
            v.modificar_diagnostico(&ate_aux, &"algo".to_string())
                .is_err_and(|e| {
        ...
        let atencion = v
            .buscar_atencion(&"Luchon".to_string(), &"Marcos".to_string(), 1234)
            .cloned();
        assert!(
            atencion.is_some_and(|a| {
                assert!(a.mascota.nombre == "Luchon");
                assert!(v.eliminar_atencion(&a).is_ok());

                assert!(
                    v.modificar_diagnostico(&a, &"algo".to_string())
                        .is_err_and(|e| {
                            assert!(!e.to_string().is_empty());
                            matches!(
                                e,
        ...
        let atencion = v
            .buscar_atencion(&"Luchito".to_string(), &"Marcos".to_string(), 1234)
            .cloned();
        assert!(
            atencion.is_some_and(|a| {
        ...
        assert!(
            v.buscar_atencion(&"Luchito".to_string(), &"Marcos".to_string(), 1234)
                .is_some_and(|a| {
                    let m = Mascota::new(&"Luchito", 2, Animales::Perro, d1.clone());
                    let ate1 =
                        Atencion::new(m, &"Vomitos", &"Pipeta", Some(Fecha::new(5, 5, 2025)));
                    a.es_igual_a(&ate1)
                }),
            "No se encontro tal recepcion"
        );
    ...
    */
    ...
    fn caso_especial_error_serde() {
        ...
        assert!(
            Veterinaria::recuperar_informacion(path_err).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
