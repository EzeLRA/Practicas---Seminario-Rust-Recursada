...
impl Display for ErroresOperatoria {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ErroresOperatoria::Denegado(val) => write!(f, "{}", val),
            ErroresOperatoria::SinVerificacion => write!(f, "Wallet sin permiso"),
            ErroresOperatoria::FalloTransaccion => write!(f, "Fallo en el proceso de transaccion"),
        }
    }
}
...
impl Criptomoneda {
    ...
    fn agregar_blockchain(&mut self, b: Blockchain) -> bool {
        ...
        if !self
            .blockchains
            .iter()
            .any(|blockchain| blockchain.es_igual_a(b.get_nombre()))
        {
    ...
    fn eliminar_blockchain(&mut self, b: &Blockchain) -> bool {
        ...
        if let Some(pos) = self
            .blockchains
            .iter()
            .position(|blockchain| blockchain.es_igual_a(b.get_nombre()))
        {
    ...
    fn blockchain_encontrado(&self, nom: &String) -> bool {
        return self
            .blockchains
            .iter()
            .any(|blockchain| blockchain.es_igual_a(nom));
    }
    fn get_blockchain(&self, nom: &String) -> Option<&Blockchain> {
        self.blockchains
            .iter()
            .find(|blockchain| blockchain.es_igual_a(nom))
    }
    fn get_nombre(&self) -> &String {
        return &self.nombre;
    }
...
impl BalancePropio {
    ...
    fn contabilizar_criptomoneda(&mut self, nom: &String, monto: f64) -> bool {
        ...
        if let Some(dato) = self
            .criptomonedas
            .iter_mut()
            .find(|cripto| cripto.es_igual_a(nom))
        {
    ...
    fn descontabilizar_criptomoneda(&mut self, nom: &String, monto: f64) -> bool {
        ...
        if let Some(pos) = self
            .criptomonedas
            .iter()
            .position(|cripto| cripto.es_igual_a(nom))
        {
    ...
    fn comprar_criptomoneda(
        ...
        if pude {
            if !self.balance.contabilizar_criptomoneda(nom, monto_a_comprar) {
                self.balance
                    .agregar_criptomoneda(nom.clone(), monto_a_comprar);
            }
        }
...
impl DatosIngreso {
    fn new(datos: DatosPersona, f: Fecha, m: f64) -> DatosIngreso {
        return DatosIngreso {
            datos_usuario: datos,
            fecha: f,
            monto: m,
...
impl InformacionPersonal for DatosIngreso {
    fn informacion_correcta(&self, info: &DatosPersona) -> bool {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct DatosRetiro {
    datos_genericos: DatosIngreso,
    medio_pago: MediosPago,
}

impl DatosRetiro {
    fn new(d: DatosPersona, fe: Fecha, monto: f64, medio: MediosPago) -> DatosRetiro {
        return DatosRetiro {
            datos_genericos: DatosIngreso::new(d, fe, monto),
            medio_pago: medio,
        };
    }
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct DatosOperacionCriptomoneda {
    datos_genericos: DatosIngreso,
    criptomoneda: Criptomoneda,
    cotizacion: f64,
}

impl DatosOperacionCriptomoneda {
    fn new(
        user: DatosPersona,
        f: Fecha,
        m: f64,
        c: Criptomoneda,
        cotiz: f64,
    ) -> DatosOperacionCriptomoneda {
        return DatosOperacionCriptomoneda {
            datos_genericos: DatosIngreso::new(user, f, m),
            criptomoneda: c,
            cotizacion: cotiz,
        };
    }
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct DatosRetiroBlockchain {
    datos_criptomoneda: DatosOperacionCriptomoneda,
    blockchain: Blockchain,
    hash: String,
}

impl DatosRetiroBlockchain {
    fn new(
        user: DatosPersona,
        f: Fecha,
        m: f64,
        ...
        cotiz: f64,
        b: Blockchain,
    ) -> DatosRetiroBlockchain {
        return DatosRetiroBlockchain {
            datos_criptomoneda: DatosOperacionCriptomoneda::new(user, f, m, c, cotiz),
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct DatosExtraccionBlockchain {
    datos_criptomoneda: DatosOperacionCriptomoneda,
    blockchain: Blockchain,
}

impl DatosExtraccionBlockchain {
    fn new(
        user: DatosPersona,
        f: Fecha,
        m: f64,
        ...
        cotiz: f64,
        b: Blockchain,
    ) -> DatosExtraccionBlockchain {
        return DatosExtraccionBlockchain {
            datos_criptomoneda: DatosOperacionCriptomoneda::new(user, f, m, c, cotiz),
            blockchain: b,
        };
    }
...
enum TiposTransacciones {
    IngresoFiat(DatosIngreso),
    CompraCriptomoneda(DatosOperacionCriptomoneda),
    VentaCriptomoneda(DatosOperacionCriptomoneda),
    RetiroCriptomoneda(DatosRetiroBlockchain),
    RecepcionCriptomoneda(DatosExtraccionBlockchain),
    RetiroFiat(DatosRetiro),
}
...
impl TiposTransacciones {
    ...
    pub fn obtener_volumen_criptomoneda(&self) -> Option<f64> {
        ...
        //Solamente se procesan los tipos compra y venta de criptomonedas , los demas se los excluye para la resolucion principal
        res = match self {
            TiposTransacciones::CompraCriptomoneda(datos) => {
                Some(datos.get_monto_operacion() * datos.get_cotizacion())
            }
            TiposTransacciones::VentaCriptomoneda(datos) => {
                Some(datos.get_monto_operacion() * datos.get_cotizacion())
            }
            _ => None,
        };
...
struct CriptomonedasDisponibles(Criptomoneda, f64);
impl CriptomonedasDisponibles {
...
struct Plataforma {
    ...
    criptomonedas_dispone: Vec<CriptomonedasDisponibles>, //Datos de la criptomoneda y cotiza
...
impl Plataforma {
    ...
    */
    ...
    fn recuperar_transacciones_info(path: &str) -> Result<Vec<TiposTransacciones>, Errores> {
        let file = File::open(path).map_err(Errores::ErrorIO)?;
        let transacciones: Vec<TiposTransacciones> =
    ...
    fn eliminar_criptomoneda(&mut self, cripto: &Criptomoneda) -> bool {
        ...
        if let Some(pos) = self
            .criptomonedas_dispone
            .iter()
            .position(|c| c.0.get_nombre() == cripto.get_nombre())
        {
            pude = true;
    ...
    fn registrar_criptomoneda(&mut self, cripto: Criptomoneda, cotiza: f64) -> bool {
        ...
        if self
            .criptomonedas_dispone
            .iter()
            .find(|&c| c.0.get_nombre() == cripto.get_nombre())
            .is_none()
        {
            self.criptomonedas_dispone
                .push(CriptomonedasDisponibles(cripto, cotiza));
            pude = true;
        }
    ...
    fn registrar_usuario(&mut self, u1: Usuario) -> Result<(), Errores> {
        if !self
            .usuarios
            .iter()
            .any(|user| user.informacion_correcta(&u1.datos))
        {
    ...
    fn retornar_fiat_usuario(&mut self, u1: &Usuario) -> f64 {
        ...
        if let Some(u) = self
            .usuarios
            .iter()
            .find(|user| user.informacion_correcta(&u1.datos))
        {
    ...
    fn comprar_criptomoneda_usuario(
        &mut self,
        u1: &Usuario,
        f: Fecha,
        monto_fiat: f64,
                ...
                .find(|&c| &c.0.get_nombre() == &nom)
            {
                ...
                if u.is_verificado() {
                    ...
                    if u.comprar_criptomoneda(nom, monto_fiat, monto_compra) {
                        ...
                        let datos = DatosOperacionCriptomoneda::new(
                            u.datos.clone(),
                            f,
                            monto_compra,
                            cr.0.clone(),
                ...
                return Err(Errores::ErrorOperatoria(ErroresOperatoria::SinVerificacion));
            }
            return Err(Errores::ErrorOperatoria(ErroresOperatoria::Inexistente(
                "Criptomonedas".to_string(),
            )));
        }
        return Err(Errores::ErrorOperatoria(ErroresOperatoria::Inexistente(
            "Wallets".to_string(),
    ...
    fn vender_criptomoneda_usuario(
        &mut self,
        u1: &Usuario,
        f: Fecha,
        criptos_vender: f64,
            ...
            .find(|user| user.informacion_correcta(&u1.datos))
        {
                ...
                .find(|&c| &c.0.get_nombre() == &nom)
            {
                ...
                if u.is_verificado() {
                    ...
                    return Err(Errores::ErrorOperatoria(ErroresOperatoria::Denegado(
                        "Saldo insuficiente de criptomonedas".to_string(),
                    )));
                }
                return Err(Errores::ErrorOperatoria(ErroresOperatoria::SinVerificacion));
            }
    ...
    fn criptomoneda_a_blockchain_usuario(
        &mut self,
        u1: &Usuario,
        f: Fecha,
        montoTransaccion: f64,
        nomCripto: &String,
        nomBlockchain: &String,
    ) -> Result<(), Errores> {
        if let Some(u) = self
            ...
            .find(|user| user.informacion_correcta(&u1.datos))
        {
            if let Some(cr) = self
                .criptomonedas_dispone
                .iter()
                .find(|&c| &c.0.get_nombre() == &nomCripto)
            {
                //Buscar blockchain en la criptomoneda
                if u.is_verificado() {
                    //Generar comprobante
                    if u.criptomoneda_a_blockchain(
                        nomCripto,
                        montoTransaccion,
                        nomBlockchain,
                        &cr.0,
                        ...
                        if let Some(b) = cr.0.get_blockchain(nomBlockchain) {
                            let datos = DatosRetiroBlockchain::new(
                                u.datos.clone(),
                                f,
                                montoTransaccion,
                                cr.0.clone(),
                    ...
                    return Err(Errores::ErrorOperatoria(
                        ErroresOperatoria::FalloTransaccion,
                    ));
                }
                return Err(Errores::ErrorOperatoria(ErroresOperatoria::SinVerificacion));
            }
    ...
    fn blockchain_a_criptomoneda_usuario(
        &mut self,
        u1: &Usuario,
        f: Fecha,
        montoTransaccion: f64,
            ...
            .find(|user| user.informacion_correcta(&u1.datos))
        {
                ...
                .find(|&c| &c.0.get_nombre() == &nomCripto)
            {
                ...
                if u.blockchain_a_criptomoneda(nomCripto, montoTransaccion, nomBlockchain, &cr.0) {
                    if let Some(b) = cr.0.get_blockchain(nomBlockchain) {
                        let datos = DatosExtraccionBlockchain::new(
                            u.datos.clone(),
                            f,
                            montoTransaccion,
                            cr.0.clone(),
    ...
    fn criptomoneda_max_monto_compras(&self) -> Option<String> {
        ...
        if !self.registro_transacciones.is_empty() {
            ...
            res = contador
                ...
                .fold(
                    None,
                    |max_actual: Option<(String, f64)>, (nom, monto)| match max_actual {
...
mod test_ejercicio5 {
    ...
    fn prueba_criptomoneda() {
        ...
        assert_eq!(
            c,
            Criptomoneda::new(&"Cripton1".to_string(), &"CRP".to_string())
        );
    ...
    fn prueba_datos_personales_usuario() {
        let datos = DatosPersona {
            nombre: "Marcos".to_string(),
            apellido: "Deltodo".to_string(),
            email: "exmpl@example.com".to_string(),
            dni: 1234876,
        };
    ...
    fn operatoria_principal_sistema() {
        ...
        assert_eq!(
            sis1.obtener_cotizacion_criptomoneda(&"Cripton".to_string()),
            1500.0
        );

        //Intentar validar un usuario inexistente
        assert!(
            sis1.validar_usuario(&Usuario::new(&"Pichu", &"Pape", &"pipi@mail.com", 33221))
                .is_err_and(|e| {
        ...
        assert!(sis1.registrar_usuario(us1.clone()).is_err_and(|e| {
            assert!(!e.to_string().is_empty());
            matches!(e, Errores::ErrorOperatoria(ErroresOperatoria::Denegado(_)))
        }));
        ...
        assert!(
            sis1.ingresar_monto_usuario(&us1, Fecha::new(13, 03, 2025), 10000.0)
                .is_ok()
        );
        assert!(
            sis1.ingresar_monto_usuario(
                &Usuario::new(
                    &"Daniel".to_string(),
                    &datos.apellido,
                    &datos.email,
        ...
        assert!(
            sis1.ingresar_monto_usuario(&us1, Fecha::new(20, 05, 2025), 10000.0)
                .is_ok()
        );
        assert!(
            sis1.comprar_criptomoneda_usuario(
                &us1,
                Fecha::new(23, 05, 2025),
                7500.0,
                c1.get_nombre()
            )
            .is_ok()
        ...
        assert!(
            sis1.vender_criptomoneda_usuario(
                &us1,
                Fecha::new(23, 05, 2025),
                500.0,
                &"Cripton2".to_string()
        ...
        assert!(
            sis1.vender_criptomoneda_usuario(&us1, Fecha::new(10, 08, 2025), 2.0, c1.get_nombre())
                .is_ok()
        );
        ...
        assert!(
            ...
            .is_err_and(|e| {
                ...
                matches!(
                    e,
                    Errores::ErrorOperatoria(ErroresOperatoria::SinVerificacion)
                )
            })
        );
        ...
        assert!(
            std::fs::remove_file("./lista_balances.json").is_ok(),
            "Error fuera de lo previsto"
        );
        assert!(
            std::fs::remove_file("./lista_transacciones.json").is_ok(),
            "Error fuera de lo previsto"
        );
    }
    ...
    fn operatoria_blockchains_sistema() {
        ...
        assert!(
            sis1.ingresar_monto_usuario(&us1, Fecha::new(20, 05, 2025), 100000.0)
                .is_ok()
        );
        ...
        assert!(
            sis1.criptomoneda_a_blockchain_usuario(
                &us3,
                Fecha::new(23, 06, 2025),
                2.0,
                c1.get_nombre(),
                &"Bloque1".to_string()
            )
        ...
        assert!(
            sis1.blockchain_a_criptomoneda_usuario(
                &us3,
                Fecha::new(27, 08, 2025),
                4.0,
                c1.get_nombre(),
                b3.get_nombre()
            )
        ...
        assert!(
            ...
            .is_err_and(|e| {
                ...
                matches!(
                    e,
                    Errores::ErrorOperatoria(ErroresOperatoria::FalloTransaccion)
                )
            })
        );
        assert!(
        ...
        //Limpieza de archivos para prevencion de exceso de los mismos
        assert!(
            std::fs::remove_file("./lista_balances2.json").is_ok(),
            "Error fuera de lo previsto"
        );
        assert!(
            std::fs::remove_file("./lista_transacciones2.json").is_ok(),
            "Error fuera de lo previsto"
        );
    }
    ...
    fn operatoria_estadistica_sistema() {
        ...
        assert!(
            sis1.ingresar_monto_usuario(&us1, Fecha::new(20, 05, 2025), 900000.0)
                .is_ok()
        );
        assert!(sis1.validar_usuario(&us1).is_ok());
        ...
        assert!(
            sis1.vender_criptomoneda_usuario(&us1, Fecha::new(25, 05, 2025), 10.0, c1.get_nombre())
                .is_ok()
        );
        assert!(
            sis1.vender_criptomoneda_usuario(&us1, Fecha::new(25, 05, 2025), 10.0, c1.get_nombre())
        ...
        assert!(
            sis1.vender_criptomoneda_usuario(&us1, Fecha::new(25, 05, 2025), 2.0, c3.get_nombre())
                .is_ok()
        );
        ...
        assert!(
            sis1.criptomoneda_max_cant_compras()
                .is_some_and(|res| res == *c3.get_nombre()),
            "Aqui no tuvo que fallar"
        );

        assert!(
            sis1.criptomoneda_max_monto_compras()
                .is_some_and(|res| res == *c1.get_nombre()),
            "Aqui no tuvo que fallar"
        );

        assert!(
            sis1.criptomoneda_max_cant_ventas()
                .is_some_and(|res| res == *c1.get_nombre()),
            "Aqui no tuvo que fallar"
        );

        assert!(
            sis1.criptomoneda_max_monto_ventas()
                .is_some_and(|res| res == *c1.get_nombre()),
            "Aqui no tuvo que fallar"
        );
        ...
        //Limpieza de archivos para prevencion de exceso de los mismos
        assert!(
            std::fs::remove_file("./lista_balances3.json").is_ok(),
            "Error fuera de lo previsto"
        );
        assert!(
            std::fs::remove_file("./lista_transacciones3.json").is_ok(),
            "Error fuera de lo previsto"
        );
    }
    ...
    fn prueba_comprobantes() {
        let d = DatosPersona {
            ...
            apellido: "fd".to_string(),
            email: "asdf".to_string(),
            dni: 123,
        };
        let c1 = DatosIngreso::new(d.clone(), Fecha::new(12, 4, 26), 100.0);
        assert!(c1.get_fecha().es_igual_a(&Fecha::new(12, 4, 26)));
        assert!(c1.get_monto() > 0.0);
        let c2 = DatosRetiro::new(
            d.clone(),
            Fecha::new(12, 4, 26),
            100.0,
        ...
        let c3 = DatosOperacionCriptomoneda::new(
            d.clone(),
            Fecha::new(1, 1, 26),
            100.0,
            Criptomoneda::new("asd", "a"),
            100.0,
        );
        ...
        let c4 = DatosExtraccionBlockchain::new(
            d.clone(),
            Fecha::new(1, 1, 26),
            10.0,
            Criptomoneda::new("asd", "a"),
        ...
        let c5 = DatosRetiroBlockchain::new(
            d,
            Fecha::new(1, 1, 26),
            1.0,
            Criptomoneda::new("asd", "a"),
    ...
    */
    ...
    fn caso_especial_error_io_balances() {
        ...
        assert!(
            sis1.registrar_usuario(us1).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
    ...
    fn caso_especial_error_io_transacciones() {
        ...
        assert!(sis1.registrar_usuario(us1.clone()).is_ok());
        assert!(
            sis1.ingresar_monto_usuario(&us1, Fecha::new(31, 2, 2026), 1000.0)
                .is_err_and(|e| {
                    assert!(!e.to_string().is_empty());
                    matches!(e, Errores::ErrorIO(_))
                }),
    ...
    fn caso_especial_error_serde() {
        ...
        assert!(
            Plataforma::recuperar_balances_info(path_err).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
        ...
        assert!(
            Plataforma::recuperar_transacciones_info(path_err).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
                matches!(e, Errores::ErrorSerde(_))
            }),
            "Aquí debió fallar"
