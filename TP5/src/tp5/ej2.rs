...
impl Display for ErroresOperatoria {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ErroresOperatoria::SinDesplazamiento(val) => write!(
                f,
                "La posicion recibida no es valida para la operacion en la estructura {} ",
                val
            ),
...
impl Cancion {
    ...
    pub fn es_igual_a(&self, c: &Cancion) -> bool {
        return (&self.titulo == c.get_titulo())
            && (&self.artista == c.get_artista())
        ...
        if !self.canciones.is_empty() {
            if pos < self.canciones.len() {
                if let Some(indice) = self
                    .canciones
                    .iter()
                    .position(|cancion| cancion.es_igual_a(c))
                {
                    if indice != pos {
            ...
            return Err(Errores::ErrorOperatoria(
                ErroresOperatoria::SinDesplazamiento(self.get_nombre().clone()),
            ));
        }
        Err(Errores::ErrorOperatoria(
            ErroresOperatoria::EstructuraVacia(self.get_nombre().clone()),
        ))
    }
...
mod testing_ejercicio8 {
    ...
    fn operatoria_canciones() {
        ...
        assert!(p.agregar_cancion(c.clone()).is_ok());
        assert!(
            p.buscar_cancion(&"pepe".to_string())
                .is_some_and(|c1| c1.es_igual_a(&c)),
        ...
        assert!(
            p.canciones.get(0).is_some_and(|c| c.es_igual_a(&c2)),
            "No existe esa cancion"
        );
        ...
        assert!(
            p.canciones.get(2).is_some_and(|c| c.es_igual_a(&c3)),
            "No existe esa cancion"
        );
        assert!(p.mover_cancion(&c2, 3).is_ok());
        assert!(
            p.canciones
                .get(p.canciones.len() - 1)
                .is_some_and(|c| c.es_igual_a(&c2)),
            "No existe esa cancion"
        );
        assert!(
            p.mover_cancion(&c2, 4).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
                matches!(
                    e,
                    Errores::ErrorOperatoria(ErroresOperatoria::SinDesplazamiento(_))
                )
            }),
            "Aqui debio fallar"
        );
        ...
        assert!(
            p.eliminar_cancion(&c3).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
                matches!(
                    e,
        ...
        assert!(
            p.mover_cancion(&c2, 0).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
                matches!(
                    e,
        ...
        assert!(
            p.eliminar_cancion(&c2).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
                matches!(
                    e,
                    Errores::ErrorOperatoria(ErroresOperatoria::EstructuraVacia(_))
    ...
    fn listado_canciones() {
        ...
        assert!(p.canciones_genero(&Generos::Rock).is_err_and(|e| matches!(
            e,
            Errores::ErrorOperatoria(ErroresOperatoria::EstructuraVacia(_))
        )));
        assert!(
            p.canciones_artista(&"Joaco".to_string())
    ...
    */
    ...
    fn caso_especial_error_io() {
        ...
        assert!(
            p.agregar_cancion(c1).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
    ...
    fn caso_especial_error_serde() {
        ...
        assert!(
            PlayList::recuperar_informacion(path_err).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
