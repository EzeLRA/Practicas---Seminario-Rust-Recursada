...
impl Display for ErroresBaja {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ErroresBaja::Inexistente(val) => {
                write!(f, "No se encontro el auto en la consecionaria {} ", val)
            }
            ErroresBaja::EstructuraVacia(val) => {
                write!(f, "La consecionaria {} no dispone de autos ", val)
            }
        }
    }
}
...
#[derive(Debug)]
struct ErroresCapacidad(String);
impl Display for ErroresCapacidad {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "La Capacidad de autos para la consecionaria {} fue superada",
            self.0
        )
    }
}
...
#[derive(Debug)]
enum Errores {
    ErrorBaja(ErroresBaja),
    ErrorExistencia,
    ErrorCapacidad(ErroresCapacidad),
    ErrorIO(io::Error),
...
impl Auto {
    ...
    pub fn es_igual_a(&self, a: &Auto) -> bool {
        return (&self.marca == a.get_marca())
            && (&self.modelo == a.get_modelo())
            && (self.anio == a.get_anio())
            && (self.precio_bruto == a.get_precio_bruto())
            && (self.color.es_igual_a(&a.get_color()));
    }
}
...
impl ConcesionarioAuto {
    ...
    pub fn es_igual_a(&self, c: &ConcesionarioAuto) -> bool {
        return (&self.nombre == c.get_nombre())
            && (&self.direccion == c.get_direccion())
            && (self.capacidad == c.get_capacidad());
    }
    //Metodos primarios
    ...
    */
    ...
    pub fn agregar_auto(&mut self, auto: Auto) -> Result<(), Errores> {
        ...
        return Err(Errores::ErrorCapacidad(ErroresCapacidad(
    ...
    pub fn eliminar_auto(&mut self, a1: &Auto) -> Result<(), Errores> {
        if !self.autos.is_empty() {
            ...
            return Err(Errores::ErrorBaja(ErroresBaja::Inexistente(
                self.get_nombre().clone(),
            )));
        }
        return Err(Errores::ErrorBaja(ErroresBaja::EstructuraVacia(
            self.get_nombre().clone(),
        )));
    }
...
mod testing_ejercicio1 {
    ...
    */
    ...
    fn creacion_consecionaria() {
        ...
        assert_eq!(
            conse1.es_igual_a(&ConcesionarioAuto::new(
                &"Conse1",
                &"Av1",
                4,
                "./lista_autos.json"
    ...
    fn operatoria_consecionaria() {
        ...
        assert!(
            conse1.agregar_auto(auto_nuevo.clone()).is_err_and(|e| {
        ...
        assert!(
            conse1.eliminar_auto(&auto_nuevo).is_err_and(|e| {
                //Validar mensaje no nulo
                assert!(!e.to_string().is_empty());
        ...
        assert!(
            conse2
                .eliminar_auto(&auto_nuevo)
                .is_err_and(|e| matches!(e, Errores::ErrorBaja(_))),
            "Aquí debió fallar el eliminar un auto en una estructura vacia"
        );

        //Agregar y buscar un nuevo auto
        let auto_nuevo = Auto::new(&"Bichito", &"j221", 2020, 1000000.0, Colores::Negro);
        assert!(
            conse2.agregar_auto(auto_nuevo.clone()).is_ok(),
            "Aqui no debio fallar"
        );

        assert!(
            conse2
                .buscar_auto(&auto_nuevo)
                .is_some_and(|a| a.es_igual_a(&auto_nuevo)),
            "Aqui no tendria que haber fallado"
        );

        assert!(
            conse2.eliminar_auto(&auto_nuevo).is_ok(),
            "Aqui no debio fallar"
        );
    ...
    */
    ...
    fn caso_especial_error_io() {
        ...
        assert!(
            conse.agregar_auto(auto).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
    ...
    fn caso_especial_error_serde() {
        ...
        assert!(
            ConcesionarioAuto::recuperar_informacion(path_err).is_err_and(|e| {
                assert!(!e.to_string().is_empty());
